...
class keithley_2001:
    ...
    def setup_temperature_measurement(self,sensor="K",slot=None,channel=None,nplc=5):
        ...
        self.write_to_dev("CONF:TEMP")
        scpi_array = self.parse_scpi_channel_string(slot,channel)
        #self.write_to_dev("ROUT:SCAN:FUNC "+scpi_array+", 'TEMP'")
    ...
    def get_value(self): # needed for compatibility
        scpi_array = self.parse_scpi_channel_string(self.slot,self.channel)
        if self.slot is None:
            answer_from_keithley = self.query("READ?")
        else:
            answer_from_keithley = self.query("TRAC:DATA?")
    ...
    def parse_scpi_channel_string(self,slot,channel): # slot is not used in this code
        if slot is None or channel is None:
            return ""
        real_slot = 1 # This device only has one slot
        scpi_array = " (@"
        if type(channel)==int:
           channel = [channel]
        for c in channel:
           if len(str(c)) == 1:
               c=str(c) # one digit channel needs to be addressed as @101 or 102,... and so on
           scpi_array=scpi_array+str(c)+","
...
if __name__ == "__main__":
    ...
    GPIB_address = 13
    keithley_2001 = keithley_2001(Prologix_port,GPIB_address)
    keithley_2001.reset()
    keithley_2001.debug = True
    keithley_2001.setup_temperature_measurement()
    time.sleep(3)
    result = []
    for i in range(10):
        a = keithley_2001.get_value()
