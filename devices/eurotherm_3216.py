...
class eurotherm_3216( minimalmodbus.Instrument ):
    ...
    def __init__(self, portname, slaveaddress=1):
        ...
        if "/" not in portname: # means instead of a serial port name, a pseudoname has been used
            if portname.lower() == "nemo": # nemo is on another computer on first serial port
                portname = "/dev/ttyS0"
                self.oven_constant = 0.000079 # calculated from exponential air-cooling curve
                self.oven_name = "nemo"
            elif portname.lower() == "enterprise":
                portname = "/dev/ttyS1"
                self.oven_constant = 9.1479e-5 # calculated from exponential air-cooling curve
                self.oven_name = "enterprise"
            elif portname.lower() == "discovery":
                portname = "/dev/ttyS5"
                self.oven_constant = 9.6224e-5 # calculated from exponential air-cooling curve
                self.oven_name = "discovery"
            elif portname.lower() == "acheron":
                portname = "/dev/ttyS2"
                self.oven_constant = 0.0000504342175227919 # calculated from exponential air-cooling curve
                self.oven_name = "acheron"
            elif portname.lower() == "surprise":
                portname = "/dev/ttyS3"
                self.oven_constant = 0.0000464082644493132 # calculated from exponential air-cooling curve
                self.oven_name = "surprise"
            elif portname.lower() == "blackpearl":
                portname = "/dev/ttyS1"
                self.oven_constant = 0.000079 # calculated from exponential air-cooling curve
                self.oven_name = "blackpearl"
            elif portname.lower() == "interceptor":
                portname = "/dev/ttyS7"
                self.oven_constant = 0.000079 # calculated from exponential air-cooling curve
                self.oven_name = "interceptor"
            elif portname.lower() == "hotplate":
                portname = "/dev/ttyS7"
                self.oven_constant = 0.000079 # calculated from exponential air-cooling curve
                self.oven_name = "hotplate"
            else:
    ...
    def get_calculated_error(self):
        """Return the error PV-SP"""
        return self.read_register(39, 0)
    ...
    def set_remote_sp(self, value):
        """Set the value of SP constantly. At least every 5s
        Args:
            value (float): Setpoint (most often in degrees)
        """
        self.write_register(26, value, 0)
        
    def set_local_trim(self, value):
        """Set the value added to SP to compensate temperature variations.
        Args:
            value (float): Setpoint (most often in degrees)
        """
        self.write_register(27, value, 0)
        
    def set_output_high_limit(self, value):
        """Set the value OP.HI
        Args:
            value (int)
        """
        self.write_register(30, value, 0)
        
    def set_output_low_limit(self, value):
        """Set the value OP.LO
        Args:
            value (int)
        """
        self.write_register(31, value, 0)
        
    def set_instrument_mode(self, value):
        """Set the value IM. 0==Auto mode (normal control) 1==Manual mode 2==Standby mode
        Args:
            value (float): Setpoint (most often in degrees)
        """
        self.write_register(199, value, 0)
        
    def set_input_sensor_type(self, value):
        """Set the value IN.TYP. 0==J-thermocouple 1==K 2==L 3==R 4==B 5==N 6==T 7==S 8==RTD 9==milivolt 10=comms input 203 11==Custom
        Args:
            value (float): Setpoint (most often in degrees)
        """
        self.write_register(12290, value, 0)
    ...
    def set_sp3(self, value):
        ...
        Args:
            value (float): Setpoint (most often in degrees)
        """
        self.write_register(164, value, 1)
    ...
    def get_setpoint_rate(self):
        ...
        return self.read_register(35, 2)   
    
    def set_setpoint_rate(self, value):
        """Set the setpoint (SP) change rate. RR.
        Args:
            value (float): Setpoint change rate (most often in degrees/minute). 0==Off (means no rate limit)
        """
        self.write_register(35, value, 2)  
    ...
    def disable_programmer_mode(self):
        """needed that the instrument does use a ramp rate"""
        No = 0
        Dwell = 1
        Programmer = 10
        self.write_register(320, No, 0)
        
    def set_ptyp_none(self):
        """needed that the instrument does use a ramp rate"""
        No = 0
        one_program = 1
        self.write_register(517, No, 0)
