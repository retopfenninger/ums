...
class keithley_2612B:
    def __init__(self, host, name=None, client_id=None):
        ...
        self.maximum_voltage_allowed = 200 # Volts
    ...
    def set_voltage(self,voltage,channel="A"):
        if channel is "A" and not self.channel_A_done:
            self.write("smua.source.levelv = "+str(float(voltage)))
        elif channel is "B" and not channel_B_done:
    ...
    def set_compliance_current(self,cc,channel="A"):
        if channel is "A" and not self.channel_A_done:
        ...
        elif channel is "B" and not self.channel_B_done:
            if cc is 0 or cc is None:
    ...
    def set_compliance_voltage(self,cc,channel="A"):
        if channel is "A" and not self.channel_A_done:
            if cc is 0 or cc is None:
    ...
    def set_current(self,current,channel="A"):
        if channel is "A" and not self.channel_A_done:
    ...
    def set_nplc(self,nplc,channel="A"):
        if channel is "A" and not self.channel_A_done:
    ...
    def get_current(self,channel="A",delay=0.01):
        if delay is not 0:
    ...
    def get_voltage(self,channel="A",delay=0.01):
        if delay is not 0:
            self.write("delay("+str(delay)+")")
        if channel is "A":
    ...
    def get_value(self,which=None,channel="A"): # needed for compatibility with keithley_6517B
        if self.what_is_measured is "current":
            A = [self.get_current(channel),time.time()]
        elif self.what_is_measured is "voltage":
            A = [self.get_voltage(channel),time.time()]
        return A
        
    def set_voltage_range(self,voltage): # needed for compatibility with keithley_6517B
        return
        
    def setup_current_measurement(self,nplc=1,channel="A"):
...
if __name__ == "__main__":
    ...
    keithley_2612B = keithley_2612B(ip_address)
    keithley_2612B.open()
    keithley_2612B.reset()
    keithley_2612B.setup_current_measurement()
    keithley_2612B.set_voltage(0.0042)
    keithley_2612B.turn_output_on()
    keithley_2612B.init()
    time.sleep(2) # wait 2 sec for the device to become ready
    result = []
    for i in range(10):
        a = keithley_2612B.get_value()
        print "Current is: ",a[0]
        result.append(a)
    keithley_2612B.turn_output_off()
    keithley_2612B.close()
    print result
