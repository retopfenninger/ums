...
class eurotherm_2404( minimalmodbus.Instrument ):
    """Instrument class for Eurotherm 2404 process controller. 
    ...
    def __init__(self, portname, slaveaddress=1):
        ...
        if "/" not in portname: # means instead of a serial port name, a pseudoname has been used
            if portname.lower() == "industria":
                portname = "/dev/industria"
                self.oven_constant = 0.000079 # calculated from exponential air-cooling curve
                self.oven_name = "industria"
            else:
    ...
    def set_setpoint_rate_limit(self, value):
        """ SPrr. RR
            value (int): 0==Off
        """
