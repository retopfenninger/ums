...
class tektronix_AFG2021:
    def __init__(self, host, name=None, client_id=None):
        self.host = host
        self.io_timeout = 2
        self.lock_timeout = 2
        self.maximum_pulse_amplitude = 5.0
    ...
    def set_pulse_duty_cycles(self,n):
        if n is not "INF" and not str(n).isdigit():
            print "Unkown number of cycles! Choose from: [INF,1,5,..(any other integer number)]"
            return
        self.write("SOUR1:PULS:DCYC "+str(n))
...
if __name__ == "__main__":
    ...
    tektronix_AFG2021 = tektronix_AFG2021(ip_address)
    tektronix_AFG2021.open()
    tektronix_AFG2021.reset()
    tektronix_AFG2021.set_burst_on()
    tektronix_AFG2021.set_burst_mode("TRIG")
    tektronix_AFG2021.set_burst_number_of_cycles(1)
    tektronix_AFG2021.set_trigger_input("EXT")
    tektronix_AFG2021.set_function_type("PULS")
    tektronix_AFG2021.set_load_impedance("INF")
    tektronix_AFG2021.set_burst_trig_delay(4.2)
    tektronix_AFG2021.set_frequency(1/(2.3))
    tektronix_AFG2021.set_waveform_polarity("NORM")
    tektronix_AFG2021.set_offset_voltage(0.5*4.4)
    tektronix_AFG2021.set_high_voltage(4.4)
    tektronix_AFG2021.set_low_voltage(0)
    tektronix_AFG2021.turn_output_on()
    tektronix_AFG2021.trigger()
    print "Pulse has been triggered"
    time.sleep(3)
    tektronix_AFG2021.trigger()
    print "And another one..."
    tektronix_AFG2021.turn_output_off()
    tektronix_AFG2021.close()
